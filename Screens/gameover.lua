...
gameover.loadButtons = true

function gameover.init()
	--als je doodgaat resetten de coins en potions weer die je hebt verzameld...
	score = 0
	potion1Counter = 0
    potion2Counter = 0
    potion3Counter = 0
	...
	if gameover.loadButtons == true then
	table.insert(buttons, newButton(
			"Play Again", 
			function ()		
				gameover.playAgain = true		
				print("Play Again")
			end))
	...
	table.insert(buttons, newButton(
			"Back to Menu", 
			function ()
				gameover.backToMenu = true
				print("Back to Menu")
			end))
		end

		gameover.loadButtons = false
end
