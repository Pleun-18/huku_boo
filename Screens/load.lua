...
load.loadButtons = true

function load.init()

	load.backToMenu = false

	font = love.graphics.newFont("Assets/Lettertypes/Amatic-Bold.ttf", 32)

	if load.loadButtons == true then
	table.insert(buttons, newButton(
			"Back to Menu", 
			function ()		
				load.backToMenu = true		
				print("Back to Menu")
			end))
		end

		load.loadButtons = false
end
...
function load.draw()
	...
	for i, button in ipairs(buttons) do
		...
		love.graphics.print("load Menu", 260, 100)
	end
end